...
#>
  ...
  param(
    ...
    [Parameter(Position=1)]                 [String]$NewName,
  ...
  Try {           # try to get the metadata from the command
  ...
  if (@($cmd).Count -ne 1) {
    Throw "Command exists more then once!`nAmbiguous reference [$Name : $CommandType]`n$($Cmd | Out-String)"
  }
  ...
  If(-Not $ExcludeHelp.IsPresent) {
    ...
    ("<#$([System.Management.Automation.ProxyCommand]::GetHelpComments((Get-Help $Name)))#>") -split "`n" | ForEach-Object {
      ...
      If ($Line -like '.*' -or ($Line -eq '<#') -or ($Line -eq '#>')){                   
        ...
        $IsCommentHelpKeyword = $True
        [void]$stringBuilder.AppendLine($Line)
      } Else {
        # return line with a tab in front        
        # do not return the empty line if they follow directly after a comment based help keyword  
        If (-not ($IsCommentHelpKeyword -and ($Line -eq ''))) {
          # return line with a tab in front
    ...
    [void]$stringBuilder.AppendLine($CommentHelpText)  # append comment based help
  }
  If ($AddParameter) {
  ...
  # Add a Indent in front of each Line
  $CommandText = $CommandText -split "`n" | ForEach-Object { "  $_`n" }
  ...
  $OutputText       # allways return the Text of the ProxyCommand
