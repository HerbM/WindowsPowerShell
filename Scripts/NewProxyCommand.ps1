...
#>
  [CmdletBinding(SupportsShouldProcess=$False, SupportsTransactions=$False,
    ConfirmImpact="None", DefaultParameterSetName="")]param(
    [Parameter(Position=0, Mandatory=$True)]  [String]$Name,
                                              [String]$NewName,
                                              [String]$Path,
    [Alias("Type")][ValidateSet('Cmdlet','Function')]
  ...
  $stringBuilder = New-Object System.Text.StringBuilder # StringBuilder offers better control over newline
  # The name of the command can be a module path like: Microsoft.Powershell.Management\Get-ChildItem
  # so we split out only the leaf namen
  $OriginCommandName = Split-Path $Name -Leaf
  # If the original command is modified, or the original comment help is included,
  # remove all links to the original command & create a flag for this
  $NoForwardHelp = $False
  ...
  Try { # get metadata from the command
    $Cmd = Get-Command -Name $Name -CommandType $CommandType -ErrorAction stop
  } catch {
    Throw $_
    Return    # exit function
  }
  if (@($cmd).Count -ne 1) { # Error if name resolve to more than 1 command
    Throw "Ambiguous Command reference [$Name : $CommandType]`n$($Cmd | Out-String)"
    Return  # exit function
  }
  ...
  if ($RemoveParameter) { 
    foreach ($ParameterName in @($RemoveParameter)) {
      [Void]$MetaData.Parameters.Remove($ParameterName) # Remove Parameters by Name from the command metadata
    }
  }
  If([String]::IsNullOrEmpty($NewName)) {  # Add comment to the Output Text
    [void]$stringBuilder.AppendLine("# Begin of ProxyCommand for command: $OriginCommandName")
  }
  If([String]::IsNullOrEmpty($NewName)) {  # Add the head of the function
    [void]$stringBuilder.AppendLine("Function $OriginCommandName {")
  } else {
    [void]$stringBuilder.AppendLine("Function $NewName {")
  }
  If(-Not $ExcludeHelp.IsPresent) {  # create comment based helptext & reformat it line by line
    ...
    ("<#$([System.Management.Automation.ProxyCommand]::GetHelpComments((Get-Help $Name)))#>") -split "`n" | ForEach-Object {
        ...
        If ($Line -like '.*' -or ($Line -eq '<#') -or ($Line -eq '#>')){
          ...
          # do not return empty lines following directly after a Help keyword
          If (-not ($IsCommendHelpKeyword -and ($Line -eq ''))) {
            # return line with indent unless comment based help keyword
  ...
  If ($AddParameter) {
    ...
    foreach ($ParameterMetadata in @($AddParameter)) {[Void]$MetaData.Parameters.Add($ParameterMetadata.Name, $ParameterMetadata)
  ...
  [void]$stringBuilder.AppendLine() # create the command sourcecode from metadata
  $CommandText = [System.Management.Automation.ProxyCommand]::create($MetaData)
  If($NoForwardHelp) {    # remove the help forwarding to the origin command
    ...
    $CommandText = $CommandText -Replace ", HelpUri='http://.*?'", '' # replace HelpUri
  }
  $CommandText = $CommandText -split "`n" | ForEach-Object { "  $_`n" } # Prefix 2 spaces on each Line
  ...
  If ($psise -and (-not $NoIseInsert.IsPresent)) {
    If(-not [String]::IsNullOrEmpty($Path)) {  # Create File ISE if Filepath was given
      '' | Out-File -FilePath $Path            # create a new File Tab in the ISE
      $File = $psise.PowerShellTabs.SelectedPowerShellTab.Files.Add($Path)
    } Else {                                   # No filepath, open File without filepath
      $File = $psise.PowerShellTabs.SelectedPowerShellTab.Files.Add()
    }
    $File.Editor.Text = $OutputText  # Internal Function, set Text in new ISE Tab
    $File.Editor.Select(1,1,1,1)     # scroll to first char
  }
  ...
  $OutputText                       # always return the Text of the ProxyCommand
