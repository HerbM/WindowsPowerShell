...
#>
  ...
  [Alias('cd','slf')]Param(
    [Parameter(ParameterSetName='Path', Position=0, ValueFromPipeline,
      ValueFromPipelineByPropertyName)][string]$Path,
  ...
  Begin {
    Set-StrictMode -Version Latest
    If (!(Get-Command LINE -CommandType Function,Alias -ea Ignore)) { 
      Function LINE { $MyInvocation.ScriptLineNumber }
    }
    Write-Verbose ("$(LINE) BEGIN ParamSet:$($PSCmdlet.ParameterSetName) " +
                   ($PSBoundParameters | Out-String))
    Write-Verbose "Invocation:~n$($MyInvocation | Out-String)"               
    $P = ''
